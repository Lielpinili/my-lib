...
local ROTATION_SPEED_GAMEPAD = Vector3.new(1, 0.77)*math.rad(4 * 60) -- (rad/s)
...
local ROTATION_SPEED_MOUSE = Vector3.new(1, 0.77)*math.rad(0.5) -- (rad/inputdelta)
local ROTATION_SPEED_POINTERACTION = Vector3.new(1, 0.77)*math.rad(7) -- (rad/inputdelta)
local ROTATION_SPEED_TOUCH = Vector3.new(1, 0.66)*math.rad(1) -- (rad/inputdelta)
...
local thumbstickCurve:(number)->number do
...
local function adjustTouchPitchSensitivity(delta: Vector3): Vector3
	...
	return Vector3.new(1, sensitivity)*delta
end
...
local function isInDynamicThumbstickArea(pos: Vector3): boolean
	...
	return 
		posTopLeft.X < pos.X and pos.X < posBottomRight.X and
		posTopLeft.Y < pos.Y and pos.Y < posBottomRight.Y
end
...
local mouseState = {
	...
	Pan = Vector3.zero, -- PointerAction
...
function CameraInput.getRotation(dt, disableKeyboardRotation: boolean?): Vector3
	local inversionVector = Vector3.new(1, UserGameSettings:GetCameraYInvertValue())
	...
	local kKeyboard =
		if disableKeyboardRotation then
			...
			Vector3.new(keyboardState.Right - keyboardState.Left, 0) * dt
	
	local kGamepad = dt*gamepadState.Thumbstick2*UserGameSettings.GamepadCameraSensitivity
...
do
	local function thumbstick(action, state, input)
		...
		gamepadState[input.KeyCode.Name] = Vector3.new(thumbstickCurve(position.X), -thumbstickCurve(position.Y))
		return Enum.ContextActionResult.Pass
	end

	local function mouseMovement(input)
		mouseState.Movement = input.Delta
	end
	...
	local touchBegan, touchChanged, touchEnded, resetTouchState do
		...
		function touchBegan(input: InputObject, sunk: boolean)
			if not (input.UserInputType == Enum.UserInputType.Touch and input.UserInputState == Enum.UserInputState.Begin) then
				warn('Input is not type.touch and state.begin')
				return
			end
		...
		function touchEnded(input: InputObject, sunk: boolean)
			if not (input.UserInputType == Enum.UserInputType.Touch and input.UserInputState == Enum.UserInputState.End) then
				warn('Input is not type.touch and state.end')
				return
			end
		...
		function touchChanged(input, sunk)
			if not (input.UserInputType == Enum.UserInputType.Touch and input.UserInputState == Enum.UserInputState.Change) then
				warn('Input is not type.touch and state.change')
				return
			end
			...
			if #unsunkTouches == 1 then
				if touches[input] == false then
					touchState.Move += input.Delta -- total touch pan movement (reset at end of frame)
				end
			end
	...
	local function pointerAction(wheel, pan, pinch, gpe)
		if not gpe then
			...
			mouseState.Pan = Vector3.new(pan.X,pan.Y,0)
	...
	function CameraInput.resetInputForFrameEnd()
		mouseState.Movement = Vector3.zero
		touchState.Move = Vector3.zero
		touchState.Pinch = 0

		mouseState.Wheel = 0 -- PointerAction
		mouseState.Pan = Vector3.zero -- PointerAction
